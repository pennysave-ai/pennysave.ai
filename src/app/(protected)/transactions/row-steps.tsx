...
const RowSteps = React.forwardRef<HTMLButtonElement, RowStepsProps>(
  (
    ...
    return (
      <nav aria-label="Progress" className="-my-4 overflow-x-auto py-4">
        <ol
          className={cn(
            "flex sm:flex-row flex-col flex-nowrap gap-x-3 justify-center",
            colors,
            className
          )}
        >
          {steps?.map((step, stepIdx) => {
            ...
            return (
              <li key={stepIdx} className="relative flex items-center pr-12">
                <button
                  key={stepIdx}
                  ref={ref}
                  aria-current={status === "active" ? "step" : undefined}
                  className={cn(
                    "group flex w-full cursor-pointer flex-row items-center sm:justify-center justify-start gap-x-3 rounded-large py-2.5",
                    stepClassName
                  )}
                ...
                >
                  ...
                  <div className="max-w-full flexs-1 text-start">
                    <div
                      className={cn(
